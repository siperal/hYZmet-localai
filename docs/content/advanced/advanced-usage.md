...
See also [chatbot-ui](https://github.com/siperal/hYZmet-localai-examples/tree/main/chatbot-ui) as an example on how to use config files.
...
A curated collection of model files is in the [model-gallery](https://github.com/siperal/hYZmet-localai/tree/master/gallery). The files of the model gallery are different from the model files used to configure LocalAI models. The model gallery files contains information about the model setup, and the files necessary to run the model locally.
